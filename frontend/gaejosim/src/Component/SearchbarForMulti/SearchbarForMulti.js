...
import "./SearchbarForMulti.css";

// TODO: searchInput guide 문구 줄지 생각해보기. http://fow.kr/multi

class SearchbarForMulti extends Component {
  state = {
    searchInput: "",
    summonerList: "",
    summonerNum: 0,
  ...
  parseSummoner = () => {
    ...
    if (this.state.searchInput !== "") {
      if (this.state.searchInput.includes("님이 방에 참가했습니다.")) {
        ...
        summonerArr = summonerArr.map((summoner) =>
          summoner.replace(/\s/g, "")
        );
        summonerArr.pop();
      } else {
  ...
  render() {
    return (
      <div className="SearchbarForMulti">
        <textarea
          className="multiInputForMulti"
          type="text"
...
export default withRouter(SearchbarForMulti);
